...
nba_salary_data_CSV = None
nba_salary_data_frame = None
memory_buffer = None
...
try :
    ...
    try :
        nba_salary_data_CSV = 'C:\\Users\\golfn\\Desktop\\python\\salaries_1985to2018.csv'
        nba_salary_data_frame = pandas.read_csv(nba_salary_data_CSV)

        memory_buffer = StringIO()
        nba_salary_data_frame.to_csv(memory_buffer, header=True, index=False, sep='|')
        ...
        try:
            cursor.copy_expert('''COPY NBA_LANDING.NBA_SALARY_DATA FROM STDIN WITH CSV HEADER DELIMITER '|';''', memory_buffer)
